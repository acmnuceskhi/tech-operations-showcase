  ...
  if (!member) {
    return (
      <main className="min-h-screen w-full flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-[clamp(2rem,6vw,3rem)] font-bold text-white mb-4 px-2">Member Not Found</h1>
          <button
          ...
          <div className="bg-black/20 backdrop-blur-sm border border-white/10 rounded-2xl p-6 sm:p-12">
            <div className="flex flex-col md:flex-row items-center md:items-start gap-6 sm:gap-8">
              ...
              <div className="flex-1 text-center md:text-left space-y-4">
                <div>
                  <h1 className="text-[clamp(1.9rem,5vw,2.5rem)] font-bold text-white mb-2 arcade-font-white px-1">{member.name}</h1>
                  {member.nickname && (
                    <p className="text-sm sm:text-base text-slate-400 mb-2 italic">{member.nickname}</p>
                  )}
                  <p className="text-sm sm:text-lg text-slate-300 font-semibold">{member.title}</p>
                </div>

                {member.description && (
                  <p className="text-slate-400 text-sm sm:text-base leading-relaxed">{member.description}</p>
                )}
          ...
          <section className="space-y-6 sm:space-y-8">
            {memberProjects.length > 0 && (
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                <h2 className="text-[clamp(1.5rem,4vw,2.1rem)] relative font-semibold text-white px-1">Contributed to</h2>
                <span className="text-slate-400 text-xs sm:text-base">{memberProjects.length} {memberProjects.length === 1 ? 'project' : 'projects'}</span>
              </div>
            )}

            {memberProjects.length === 0 ? (
              <div className="text-center py-10 sm:py-16 bg-black/10 rounded-xl border border-white/10">
                <p className="text-slate-400 text-sm sm:text-base">No projects yet</p>
              </div>
            ) : (
